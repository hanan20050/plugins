...
def parse_regions_for_players():
    ...
    for world in os.listdir(base_dir):
        ...
        for i in range(1, len(regions_raw), 2):
            ...
            r_body = recipes_raw = regions_raw[i+1]
...
def generate_report():
    ...
    artifact_path = os.path.join(artifact_dir, "rayan_azan_audit_report.md")
    
    lines = []
    lines.append("# Detailed Audit Report: Rayan Saleh & Azan Saleh")
    lines.append(f"**Generated:** `{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`\n")
    
    for p_key in ["rayan saleh", "azan saleh"]:
