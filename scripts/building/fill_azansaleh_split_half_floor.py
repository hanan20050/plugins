#!/usr/bin/env python3
"""
Fill Azan Saleh's region base floor (Y=62) split into 2 distinct concrete halves:
- North Half (Z: -219 to -205, X: 1241..1255): Cyan Concrete
- South Half (Z: -204 to -191, X: 1241..1255): White Concrete
...
BACKUP_FILE = "azansaleh_split_half_floor_history.json"
...
def apply_split_half_floor(color1="minecraft:cyan_concrete", color2="minecraft:white_concrete", dry_run=False):
    if not color1.startswith("minecraft:"):
        color1 = f"minecraft:{color1}"
    if not color2.startswith("minecraft:"):
        color2 = f"minecraft:{color2}"

    print(f"Filling Azan's floor (Y={Y}) in 2 distinct concrete halves ({color1} & {color2})...")
    ...
    if not dry_run:
        ...
        print(f"Floor updated successfully! Saved history to {BACKUP_FILE}")

def undo(dry_run=False):
    print("Undoing split half floor (restoring floor to uniform white_concrete)...")
...
def main():
    parser = argparse.ArgumentParser(description="Fill Azan Saleh's floor into 2 concrete halves")
    parser.add_argument("--color1", default="minecraft:cyan_concrete", help="North half concrete color")
    parser.add_argument("--color2", default="minecraft:white_concrete", help="South half concrete color")
    parser.add_argument("--dry-run", action="store_true", help="Print commands without executing")
    parser.add_argument("--undo", action="store_true", help="Undo floor split")
    args = parser.parse_args()

    if args.undo:
        undo(dry_run=args.dry_run)
    else:
        apply_split_half_floor(color1=args.color1, color2=args.color2, dry_run=args.dry_run)
