...
- **Currency Exchange & Sales Rule (STRICT)**: NEVER apply sales or discounts to Money Exchange or currency exchange trades (e.g. trading Emeralds <-> Emerald Blocks, Netherite Ingots, or Netherite Blocks) UNLESS explicitly asked by the user. Currency exchange rates are strictly fixed (1 Netherite Ingot = 64 Emeralds, 1 Emerald Block = 9 Emeralds, 1 Netherite Block = 576 Emeralds) and must never be altered during any sale or offer.
...
- **Trade Money Confirmation Rule (STRICT)**: Whenever creating, modifying, adding, or executing any trade that yields money/currency (e.g. trading items for Emeralds, Emerald Blocks, Netherite Ingots, or Netherite Blocks), ALWAYS ask the user for explicit confirmation before applying or executing the change.
